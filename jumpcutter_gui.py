...
def FText(
    ...
    """
    A "Fixed-sized Text Input".
    Returns a row with a Text and an Input element.
    """
    ...
    return [
        ...
        sg.Input(
            default_text=default, key=in_key, size=input_size, tooltip=tooltip
        ),
    ]
...
def main():
    ...
    # This version of the GUI uses this large dictionary to drive 100%
    # of the creation of the layout that collections the parameters for
    # the command line call.  It's really simplistic
    ...
    input_defintion = {
        ...
        "-OUT FILE-": (
            ...
            "",
            (40, 1),
            "the output file. (optional. if not included, it'll just modify"
            " the input file name)",
            [sg.FileSaveAs()],
        ),
        "-SILENT THRESHOLD-": (
        ...
        "-FRAME RATE-": (
            ...
            30,
            None,
            "frame rate of the input and output videos. optional... I try to"
            " find it out myself, but it doesn't always work.",
            [],
        ),
        "-FRAME QUALITY-": (
            "--frame-quality",
            ...
            3,
            None,
            "quality of frames to be extracted from input video. 1 is highest,"
            " 31 is lowest, 3 is the default.",
            [],
        ),
    }

    # the command that will be invoked with the parameters
    command_to_run = r"python .\main.py "
    ...
    # Computed part of layout that's based on the dictionary of attributes
    # (the table driven part)
...
def runCommand(cmd, timeout=None, window=None):
    ...
    @param window: the PySimpleGUI window that the
        output is going to (needed to do refresh on)
